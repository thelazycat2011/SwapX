...
ImVec4 ccToImVec(cocos2d::ccColor4B& c) {
...
void setup() {
    ...
    float globalRound = Mod::get()->getSettingValue<float>("rounding"); // screw it let the user f it themselves
    ...
    // auto titlecolor = Mod::get()->getSettingValue<cocos2d::ccColor4B>("titlecol");
    ...
    colors[ImGuiCol_TitleBg] = colors[ImGuiCol_TitleBgActive] = ImVec4(0.0f, 0.0f, 0.5f, 1.0f); // nvm
...
void draw() {
    ...
    if (ImGui::Begin("SwapX", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize)) {
        ...
        if (ImGui::Button("Refresh")) g.lvllistRefresh();
        ImGui::Checkbox("Swapping", &g.swap);
        ImGui::Checkbox("Only swap on specified ID", &g.swapID);
        ImGui::InputText("ID", g.id, 15, ImGuiInputTextFlags_CharsDecimal); // future proofing
        ImGui::End();
    }
}
